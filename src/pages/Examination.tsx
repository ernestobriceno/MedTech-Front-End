...
export const Examination: React.FC = () => {
  ...
  if (patient) {
    ...
    React.useEffect(() => {
      fetch(import.meta.env.VITE_API_URL + "/users/files?category=examinations", {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      })
  ...
  const handleAddFile = () => {
    if (selectedFile) {
      ...
      formData.append("category", "examinations");
      ...
      <header className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <h1 className="text-lg font-bold">Examination files</h1>
          </div>
        </div>
      </header>
